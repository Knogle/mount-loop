...
setup_faulty_loop_device() {
    ...
    TOTAL_SIZE=$(sudo blockdev --getsize64 "$LOOPDEVICE")
    ...
    echo -e "$DM_TABLE" | sudo dmsetup create "$DM_DEVICE_NAME"

    MAPPED_DEVICE="/dev/mapper/$DM_DEVICE_NAME"
    echo "Faulty loop device created: $MAPPED_DEVICE"

    if [ "$CREATE_FS" = true ]; then
        sudo mkfs.ext4 "$MAPPED_DEVICE" >/dev/null 2>&1
        if [ $? -ne 0 ]; then
            echo "Failed to create filesystem on $MAPPED_DEVICE"
            sudo dmsetup remove "$DM_DEVICE_NAME"
            sudo losetup -d "$LOOPDEVICE"
            exit 1
        fi

        MOUNTPOINT=$(mktemp -d)
        sudo mount "$MAPPED_DEVICE" "$MOUNTPOINT"
        if [ $? -ne 0 ]; then
